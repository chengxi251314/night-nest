...
import { generateAuthToken, getTokenUserId, getUserIdFromAuth } from "./auth-helper";
...
export class AuthController {
  ...
  async me(@Headers("authorization") auth: string) {
    ...
    if (!userId || userId === "demo-user") return { user: null };
    try {
  ...
  async updateProfile(@Headers("authorization") auth: string, @Body() body: { nickname?: string; gender?: string; bio?: string; avatar?: string }) {
    const userId = getUserIdFromAuth(auth);
    if (!userId || userId === "demo-user") return { error: "Not logged in" };
    try {
      const sets: string[] = [];
      const vals: any[] = [];
  ...
  async uploadAvatar(@Req() req: any, @Res() res: any) {
    try {
      ...
      const storage = multer.diskStorage({
        ...
        filename: function(_req: any, file: any, cb: any) { const ext = path.extname(file.originalname) || ".png"; cb(null, "avatar-" + Date.now() + "-" + Math.random().toString(36).slice(2, 5) + ext); }
      });
